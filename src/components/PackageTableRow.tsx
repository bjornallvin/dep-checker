...
export const PackageTableRow = ({
  ...
  const index = packages.findIndex(
    (pkg) => pkg.packageName === packageName && pkg.projectName === projectName
  );

  const [latestVersion, setLatestVersion] = useState<string>(
    packages[index].latestVersion || "?"
  );

  useEffect(() => {
    const updateOtherPackages = () => {
    ...
    if (!packages[index].latestVersion) {
      getLatestVersion(packageName).then((version) => {
        packages[index].latestVersion = version;
        setLatestVersion(version);
        updateOtherPackages();
      });
    }
  }, [packageName, lockedVersion, packages, index, latestVersion]);
  ...
  return (
    <tr key={projectName + packageName + wantedVersion} className={`border-b `}>
      ...
      <td className={" pr-2"}>
        {latestVersion !== "?" ? (
          <div className={`${getColor(lockedVersion, latestVersion)} `}>
            {latestVersion}
          </div>
        ) : (
          <div role="status">
            <svg
              aria-hidden="true"
              className="w-5 h-5 mr-2 text-base-100 animate-spin fill-accent"
              viewBox="0 0 100 101"
              fill="none"
